...
plot.example <- function(g, .examples, .res = 1e4) {
    ...
    .dt.sum <- .dt[,
                   .(log10.pval = pmin(mean(-log10(p.val)), 30),
                     log10.pval.lb = pmin(min(-log10(p.val)), 30),
                     log10.pval.ub = pmin(max(-log10(p.val)), 30),
                     lodds = max(lodds)),
                   by = .(loc = floor(`stop`/.res),
                          celltype)]
    ...
    .hex <- data.table(celltype = sort(unique(.dt.sum$celltype))) %>%
        left_join(ct.order.dt) %>%
        select(hex) %>%
        unlist
    ...
    p2 <-
        ...
        geom_segment(aes(xend = loc, y = -log10.pval.lb, yend = -log10.pval.ub, colour = celltype), size=.1) +
        scale_colour_manual(values = .hex, guide = "none") +
        geom_vline(aes(xintercept = loc), data = .dt.sum[lodds > 0],
                   colour = "red", size = .2)
