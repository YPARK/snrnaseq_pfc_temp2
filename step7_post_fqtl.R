...
.out.chr.file <- str_c("share/2021-10-29/fqtl/", .factor, "/chr", .chr, ".geno.bed.gz")
...
if(!file.exists(.out.chr.file)) {
    ...
    .files <- str_c("result/step7/fqtl/", .genes, "_stat.bed.gz")
...
.out.chr.file <- str_c("share/2021-10-29/fqtl/", .factor, "/chr", .chr, ".celltype.bed.gz")
.mkdir(dirname(.out.chr.file))

if(!file.exists(.out.chr.file)){
    .files <- str_c("result/step7/fqtl/", .genes, "_celltype.bed.gz")
    ...
    .dt.chr <-
        lapply(.files, function(x){
            ...
            g <- basename(x) %>% str_remove("_celltype.bed.gz$") %>% as.integer
            log.msg("[%05d / %05d]", g, max.g)
